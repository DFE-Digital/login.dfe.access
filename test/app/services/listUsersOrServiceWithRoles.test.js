...
jest.mock('./../../../src/infrastructure/data', () => ({
  getUsersOfServicePagedV2: jest.fn(),
}));
const { mockRequest, mockResponse } = require('./../../utils');

const { getUsersOfServicePagedV2 } = require('./../../../src/infrastructure/data');
const listUsersOfServiceWithRoles = require('./../../../src/app/services/listUsersOfServiceWithRoles');
...
describe('when listing users of service within organisation', () => {
  ...
  beforeEach(() => {
    getUsersOfServicePagedV2.mockReset().mockReturnValue({
      services: [
        {
          userId: 'user-1',
          serviceId: 'service-1',
    ...
    req = mockRequest({
      ...
      query: {
        page: '2',
        pageSize: '50',
        roleIds: 'E53644D0-4B4A-43BD-92A9-F019EC63F978'
      },
    });
  ...
  it('then it should return json page of data', async () => {
    await listUsersOfServiceWithRoles(req, res);
  ...
  it('then it should use page settings from query string if present', async () => {
    ...
    expect(getUsersOfServicePagedV2).toHaveBeenCalledTimes(1);
    expect(getUsersOfServicePagedV2).toHaveBeenCalledWith('service-1', 'organisation-1', ["E53644D0-4B4A-43BD-92A9-F019EC63F978"], 2, 50);
  });
  ...
  it('then it should default to page 1 if no page specified', async () => {
    ...
    expect(getUsersOfServicePagedV2).toHaveBeenCalledTimes(1);
    expect(getUsersOfServicePagedV2).toHaveBeenCalledWith('service-1', 'organisation-1', ["E53644D0-4B4A-43BD-92A9-F019EC63F978"], 1, 50);
  });
  ...
  it('then it should default to page size of 25 if no page size specified', async () => {
    ...
    expect(getUsersOfServicePagedV2).toHaveBeenCalledTimes(1);
    expect(getUsersOfServicePagedV2).toHaveBeenCalledWith('service-1', 'organisation-1', ["E53644D0-4B4A-43BD-92A9-F019EC63F978"], 2, 25);
  });
