...
const validateRequest = (req) => {
  const model = {
    ...
    status: req.body.status && req.body.status.id !== undefined ? req.body.status.id : undefined,
...
const updatePolicy = async (model, existingPolicy) => {
  ...
  await addPolicy(updatedPolicy.id, updatedPolicy.name, updatedPolicy.applicationId, updatedPolicy.status.id);
};
  ...
  if (model.conditions) {
    ...
    for (let i = 0; i < model.conditions.length; i++) {
      ...
      for (let j = 0; j < condition.value.length; j += 1) {
        await addPolicyCondition(uuid.v4(), model.id, condition.field, condition.operator, condition.value[j]);
      }
    }
  }
};
...
const updatePolicyOfService = async (req, res) => {
  ...
  try {
    ...
    if (!existingPolicy || existingPolicy.applicationId.toLowerCase() !== model.applicationId.toLowerCase()) {
