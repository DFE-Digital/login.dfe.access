...
const listAllInvitationsServices = async (req, res) => {
  ...
  logger.debug(
    `Listing page ${pageNumber} of invitation services (pageSize: ${pageSize}`,
    { correlationId },
  );
  try {
    const invitationServices =
      (await getPageOfInvitationServices(pageNumber, pageSize)) || [];
    ...
    logger.error(
      `Error listing page ${pageNumber} services (pageSize: ${pageSize}`,
      {
        correlationId,
        error: { ...e },
      },
