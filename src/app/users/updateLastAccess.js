...
const updateLastAccess = async (req, res) => {
  ...
  try {
    ...
    await updateUserServiceLastAccess(uid, sid, oid);

    notifyUserUpdated(uid);
    return res.status(202).send();
  } catch (e) {
    logger.error(`Error updating last access for service ${sid} with org ${oid} to user ${uid} (correlation id: ${correlationId}) - ${e.message}`, {
      correlationId,
      stack: e.stack,
    });
    throw e;
